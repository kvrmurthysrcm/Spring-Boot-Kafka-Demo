...
public class KafkaMessagePublisher {
    ...
    public void sendProductToTopicPartition(Product product) {
        System.out.println("@Service:: @sendProductToTopic(" + product.toString() + ")");
        String KEY = "SpringBoot-Kafka-App";
        for(int i = 5; i < 1000; i++) {
            Integer partition = i % 5; // topic partition number
        // publish the message to the TOPIC on Partition 0,1,2,3,4
        CompletableFuture<SendResult<String, Object>> future = template.send(PRODUCT_TOPIC, partition, KEY, product );
        // Exception/logs processing
        future.whenComplete((result, ex) ->{
            if(ex == null) {
                System.out.println("Send message=[" + product.toString() +
